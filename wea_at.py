...
def generate_data(data, start, end, SIZE_OF_WINDOW):
    ...
    for i in range(1, end - start + 2 - SIZE_OF_WINDOW):
        ...
        for j in range(len(X)):
            ...
            if not math.isnan(k.predict([51.22629, 6.76808])):
                #print(k.predict([51.22629, 6.76808]))
                X[j,2] = k.predict([51.22629, 6.76808])
...
data1 = data.where(data["station_no"] == 6335050).dropna(axis = 0)
...
pickle.dump(X, open('x_delta_50.pkl', 'wb'))
pickle.dump(Y, open('y_delta_50.pkl', 'wb'))


#X = pickle.load(open('x_delta_50.pkl', 'rb+'))
#Y = pickle.load(open('y_delta_50.pkl', 'rb+'))
#print(X)
class lstm(nn.Module):
    def __init__(self,input_size = 1, hidden_size = 200, num_layer=2):
...
model = model_lstm.lstm(20, 1, 50, 0, 0, False, 0.2, True, 27, 27, 3)
...
for i in range(epochs):
    ...
    if best1 < best:
        best = best1
        torch.save(model, "./models_window_" + str(SIZE_OF_WINDOW) +"attention_delta_50.pth")
        d = np.array(save)
